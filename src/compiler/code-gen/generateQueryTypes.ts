...
export default function generateQueryTypes(ast: AST): string {
  ...
  Object.keys(classes).forEach(name => {
    ...
    result.push(
      `export class ${name}Query<TResult = {}> extends ${name === 'Root'
        ? imports.get('BaseRootQuery')
        : imports.get('BaseQuery')}<TResult> {`,
    );
    result.push(`  // fields`);
    properties.forEach(({propertyName, argsType, resultType}) => {
      ...
      const genericsString = generics.length ? `<${generics.join(', ')}>` : '';
    ...
    Object.keys(type.staticAPI.methods).forEach(mutationName => {
      ...
      args.push(
        `optimisticUpdate?: (mutation: {objectName: '${name}'; methodName: '${mutationName}'; args: ${generateType(
          ...
          'RootCache',
        )}, getOptimisticValue: ${imports.get('GetOptimisticValue')}) => any`,
      );
      result.push(
